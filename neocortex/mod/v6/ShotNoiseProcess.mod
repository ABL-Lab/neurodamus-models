COMMENT
/**
 * @file ShotNoiseProcess.mod
 * @brief 
 * @author dhuruva(gmdhuruva@gmail.com)
 * @date 2026-02-12
 */
ENDCOMMENT
...
COMMENT
Poisson Shot noise model intended to replace ProbAMPANMDA_EMS's AMPA component of multiple synapses.
The model approximates g0 and std as noise, where g0 and std are calculated based on Richardson's formula
that can be found in ShotNoiseProcessHelper.hoc. 
...
ASSIGNED {
    ...
    rate    (1/ms)
    q       (uS)
    gain            : multiplicative weight from NetCon
    usingR123
    rng
}
...
FUNCTION urand() {
    ...
    if (value != value) { value = 0.5; }
    if (value < 0.0 || value > 1.0e308) { value = 0.5; }
